...
namespace VF.Model
{
  ...
  {
    public static implicit operator GuidAnimationClip(AnimationClip d) => new GuidAnimationClip { setter = d };
  }

  [Serializable]
  public class GuidMaterial : GuidWrapper<Material>
  {
    public static implicit operator GuidMaterial(Material d) => new GuidMaterial { setter = d };
  }

  [Serializable]
  public class GuidTexture2d : GuidWrapper<Texture2D>
  {
    public static implicit operator GuidTexture2d(Texture2D d) => new GuidTexture2d { setter = d };
  }

  [Serializable]
  public class GuidController : GuidWrapper<RuntimeAnimatorController>
  {
    public static implicit operator GuidController(RuntimeAnimatorController d) => new GuidController { setter = d };
  }

  [Serializable]
  public class GuidMenu : GuidWrapper<VRCExpressionsMenu>
  {
    public static implicit operator GuidMenu(VRCExpressionsMenu d) => new GuidMenu { setter = d };
  }

  [Serializable]
  public class GuidParams : GuidWrapper<VRCExpressionParameters>
  {
    public static implicit operator GuidParams(VRCExpressionParameters d) => new GuidParams { setter = d };
  }
